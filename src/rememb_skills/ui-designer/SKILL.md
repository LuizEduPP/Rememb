...
Use an isolated worker or equivalent task runner when available, providing:
...
Attach reference images to the worker context.
...
- Replace `{project-background}` with content from the project idea file
...
**Substitutions:**
- `{project-design-guide}` -> Design system from Step 2
- `{project-mvp-prd}` -> PRD from Step 3 or provided PRD file
